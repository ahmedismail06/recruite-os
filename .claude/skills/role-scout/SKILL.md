...
**No thesis given → derive one.** If Ahmed invokes the skill without a thesis (or asks "what roles should I even look for"), load the Profile Bank first and propose 2–4 candidate theses derived from it — each a one-line search thesis plus 1–2 sentences of reasoning grounded in actual bank rows (which projects/skills make this a strong lane). Present them, let Ahmed pick or edit one (multiple is fine), and only then continue to sourcing. Derived theses follow the same honesty rules as fit notes: grounded in real bank content, gaps acknowledged, no flattery.
